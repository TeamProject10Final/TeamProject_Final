...
import com.example.donotlate.feature.searchPlace.domain.model.ResultEntity

fun GooglePlace.toEntity(): GooglePlacesEntity {
    return GooglePlacesEntity(htmlAttributions, nextPageToken, results.map { resultReponse -> resultReponse.toEntity() } , status)
}

fun ResultReponse.toEntity(): ResultEntity {
    return ResultEntity(businessStatus, icon, iconBackgroundColor, iconMaskBaseUri, name, openingHours.toEntity(), photos.map { it.toEntity() }, placeId, priceLevel, rating, types, userRatingsTotal)
}
